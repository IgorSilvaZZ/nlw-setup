...
export const NewHabitForm = () => {
  ...
  function handleToggleWeekDay(weekDay: number) {
    if (weekDays.includes(weekDay)) {
      const weekDsyWithRemovedOne = weekDays.filter((day) => day !== weekDay);

      setWeekDays(weekDsyWithRemovedOne);
    } else {
  ...
  return (
    <form onSubmit={createNewSubmit} className='w-full flex flex-col mt-6'>
      ...
      <input
        type='text'
        id='title'
        placeholder='ex.: Exercicios, domir bem, etc..'
      ...
      <button
        type='submit'
        className='mt-6 rounded-lg p-4 flex gap-3 items-center justify-center font-semibold bg-green-600 transition-colors hover:bg-green-500 focus:outline-none focus:ring-2 focus:ring-green-600 focus:ring-offset-2 focus:ring-offset-zinc-900'
      >
